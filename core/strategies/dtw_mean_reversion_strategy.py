...
class DTWMeanReversionStrategy(BaseStrategy):
    ...
    def __init__(self, config: Dict):
        ...
        self.capital = config.get('capital', 100000)  # Default to $100k if not set
    ...
    def _passes_volume_filter(self, symbol: str) -> bool:
        ...
        return current_volume >= avg_volume * self.min_volume_ratio    def _check_multi_timeframe_confirmation(self, symbol: str,
    ...
    def _generate_signal(self, symbol: str, df: pd.DataFrame,
        ...
        if '1h' not in confirmation['details']:
            return None
        analysis = confirmation['details']['1h']
        ...
        return signal    def _calculate_confidence(self, analysis: Dict, confirmation: Dict) -> float:
        ...
        volatility_factor = 1.0 - min(1.0, analysis['volatility'] / 0.1)
    ...
    def _calculate_position_size(self, entry_price: float, stop_loss: float,
        ...
        size = base_size * (0.1 / max(0.01, volatility))
    ...
    def save_state(self) -> Dict[str, Any]:
        return {
            ...
            'last_signals': [asdict(s) for s in self._signal_history[-100:]],
            'config': {
